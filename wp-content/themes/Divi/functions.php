...
/**
 * Validar si el ambiente es stage.
 *
 * @return boolean
 */
function environment_is_stage()
{
	return strtolower(getenv('APP_ENV')) === 'stage';
}

/**
 * Validar si el ambiente es producción.
 *
 * @return boolean
 */
function environment_is_prod()
{
	return strtolower(getenv('APP_ENV')) === 'prod';
}

/**
 * Obtener el id del google tag manager de acuerdo al ambiente.
 *
 * @return string
 */
function get_gtm_id()
{
	if (environment_is_stage()) {
		return 'GTM-5KH72N4';
	}
...
/**
 * Script de google tag manager para aplicar en la cabecera.
 *
 * @return void
 */
function add_gtm_to_header()
{
	if (environment_is_stage() || environment_is_prod()) { ?>
	<!-- Google Tag Manager -->
	<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
	new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
	j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
	'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
	})(window,document,'script','dataLayer','<?php echo get_gtm_id();?>');
	</script>
...
/**
 * Agregar entrada en la cabecera de las paǵinas publicas
 */
add_action('wp_head', 'add_gtm_to_header', 1);
/**
 * Agregar entrada en la cabecera de las paǵinas privadas
 */
add_action('admin_head', 'add_gtm_to_header', 1);

/**
 * NoScript de google tag manager para aplicar en el body.
 *
 * @return void
 */
...
{	
    ...
    <noscript><iframe src="https://www.googletagmanager.com/ns.html?id=<?php echo get_gtm_id();?>"
...
/**
 * Agregar entrada en el cuerpo de las paǵinas publicas
 */
add_action('wp_body_open', 'add_gtm_to_body', 1);
/**
 * Agregar entrada en el cuerpo de las paǵinas privadas
 */
add_action('in_admin_header', 'add_gtm_to_body', 1);

/**
 * Script de clarity para aplicar en la cabecera.
 *
 * @return void
 */
function add_clarity_to_header()
{
	if (environment_is_stage() || environment_is_prod()) { ?>
	<!-- Clarity -->
	<script type="text/javascript">(function(c,l,a,r,i,t,y){ c[a]=c[a]||function(){(c[a].q=c[a].q||[]).push(arguments)}; t=l.createElement(r);t.async=1;t.src="https://www.clarity.ms/tag/"+i; y=l.getElementsByTagName(r)[0];y.parentNode.insertBefore(t,y); })(window, document, "clarity", "script", "iu1ezjmx2u");</script>
	<!-- End Clarity -->
	<?php }
}

/**
 * Agregar entrada en la cabecera de las paǵinas públicas
 */
add_action('wp_head', 'add_clarity_to_header', 2);
/**
 * Agregar entrada en la cabecera de las paǵinas privadas
 */
add_action('admin_head', 'add_clarity_to_header', 2);
